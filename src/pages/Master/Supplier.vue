<template>
  <q-page class="tw-p-4 tw-space-y-6">
    <div class="tw-text-xl tw-font-semibold">Supplier</div>
    <q-table
      color="primary"
      :columns="columns"
      :rows="rows"
  ...
  <q-dialog v-model="form_dialog">
    <q-card style="min-width: 400px">
      <q-card-section class="row items-center">
        <div class="text-h6">{{ !is_edit ? "Create" : "Edit" }} Supplier</div>
        <q-space />
...
export default defineComponent({
  ...
  methods: {
    ...
    submit() {
      ...
      if (!this.is_edit) {
        this.$api
          .post("/suppliers", { ...this.form, created_by: this.user.id })
          .then((res) => {
            this.$q.notify({
